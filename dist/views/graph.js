...
d3.json('/data', function (err, data) {
    ...
    node.append('circle')
        ...
        .style('fill', 'white')
        .attr('r', 40)
        .attr('cx', function (d, i) { return Math.min(750, Math.random() * (i + 2) * 100); })
        .attr('cy', function (d, i) { return Math.min(550, Math.random() * (i + 2) * 100); })
        .on('mouseover', function () { d3.select(this).style('fill', 'aliceblue'); })
        .on('mouseout', function () { d3.select(this).style('fill', 'white'); });
    node.append('text')
        .attr('font', '10px sans-serif')
        .attr('x', function (d, i) { return d3.select('#node-' + d.id).select('circle').attr('cx'); })
        .attr('y', function (d, i) { return d3.select('#node-' + d.id).select('circle').attr('cy'); })
    ...
    var link = baseSVG.selectAll('.link')
        ...
        .attr('class', 'link');
    link.append('line')
        .attr('x1', function (d) { return d3.select('#node-' + d.from).select('circle').attr('cx'); })
        .attr('y1', function (d) { return d3.select('#node-' + d.from).select('circle').attr('cy'); })
        .attr('x2', function (d) { return d3.select('#node-' + d.to).select('circle').attr('cx'); })
        .attr('y2', function (d) { return d3.select('#node-' + d.to).select('circle').attr('cy'); })
        .attr('stroke', '#ccc');
});
